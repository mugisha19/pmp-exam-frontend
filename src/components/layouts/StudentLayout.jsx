...
export const StudentLayout = () => {
  ...
  return (
    <div className="flex min-h-screen bg-transparent relative">
      ...
      <aside className="hidden lg:flex flex-col fixed left-4 top-1/2 -translate-y-1/2 w-64 h-[92vh] bg-white/90 backdrop-blur-xl shadow-md shadow-gray-200/40 rounded-2xl border border-gray-200/60 z-50 overflow-hidden">
        <div className="flex flex-col h-full">
          {/* Logo & Brand */}
          <div className="p-5 pb-4 border-b border-gray-100">
            <div className="flex items-center gap-3">
              <div className="relative">
                <div className="flex items-center justify-center w-11 h-11 rounded-xl shadow-sm ring-2" style={{ backgroundColor: '#476072', ringColor: 'rgba(71, 96, 114, 0.1)' }}>
                  <GraduationCap className="w-5 h-5 text-white" />
                </div>
                <div className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full border-2 border-white shadow-sm" style={{ backgroundColor: '#476072' }}></div>
              </div>
              <div>
                <h1 className="text-lg font-semibold bg-clip-text text-transparent" style={{ background: 'linear-gradient(135deg, #53629E 0%, #6b7aad 100%)', WebkitBackgroundClip: 'text' }}>
                  PMP Portal
                </h1>
                <p className="text-[10px] text-gray-600 font-medium mt-0.5 uppercase tracking-wide">Student Dashboard</p>
              </div>
            </div>
          </div>
            ...
            {STUDENT_NAV_ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-center w-9 h-9 transition-all duration-200">
                    <Icon className={cn(
                  ...
                  <span className={cn(
                    "text-xs font-medium transition-all duration-200 flex-1 text-left",
          ...
          {/* Profile Section */}
          <div className="mt-auto p-4 border-t border-gray-100 bg-gradient-to-br from-gray-50/50 via-white to-gray-50/30 rounded-b-2xl">
            {/* Notifications */}
            <button 
              onClick={() => navigate("/notifications")}
              className={cn(
                "group w-full flex items-center gap-3 px-3 py-2.5 mb-3 rounded-xl transition-all duration-200",
            ...
            >
              <div className="relative">
                ...
                {unreadCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-5 h-5 bg-red-500 text-white text-[10px] font-semibold rounded-full flex items-center justify-center border border-white">
                    {unreadCount > 9 ? "9+" : unreadCount}
                  </span>
                )}
              </div>
              <span className={cn(
                "text-xs font-medium transition-colors duration-200 flex-1 text-left",
            ...
            <div className="relative">
              ...
              >
                <div className="relative">
                  <div className="w-9 h-9 rounded-lg flex items-center justify-center shadow-sm ring-2" style={{ backgroundColor: '#476072', ringColor: 'rgba(71, 96, 114, 0.1)' }}>
                    <span className="text-white font-semibold text-xs">
                      {user?.first_name?.[0]}{user?.last_name?.[0]}
                    </span>
                  </div>
                  <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white shadow-sm" style={{ backgroundColor: '#476072' }}></div>
                </div>
              ...
              {/* Profile Dropdown */}
              {profileMenuOpen && (
                <div className="absolute bottom-full left-0 right-0 mb-2 bg-white rounded-xl shadow-md border border-gray-200 py-1.5 overflow-hidden backdrop-blur-xl animate-in fade-in slide-in-from-bottom-2 duration-200">
                  <button
                    onClick={() => {
                      navigate("/profile");
                      setProfileMenuOpen(false);
                    }}
                    className="w-full px-3 py-2 text-left text-xs text-gray-700 hover:bg-gray-50 flex items-center gap-3 transition-all duration-200 group"
                  >
                    <div className="w-8 h-8 rounded-lg bg-gray-100 group-hover:bg-gray-200 flex items-center justify-center transition-all duration-200">
                  ...
                  <button
                    ...
                    className="w-full px-3 py-2 text-left text-xs text-red-600 hover:bg-red-50 flex items-center gap-3 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed group"
                  >
                    <div className="w-8 h-8 rounded-lg bg-red-50 group-hover:bg-red-100 flex items-center justify-center transition-all duration-200">
